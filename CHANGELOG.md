...
- c6bf0c7: feat(cli): add `--base-dir` to relocate the project root
...
- e7a3844: fix(core): correct version resolution for globally-installed skills
...
- 3097399: feat(core): persist source metadata for global installs and support `update -g`
...
- d5cf6ac: feat(outdated): support registry source detection and global skill checking
...
- 373a704: Fix Windows registry tarball extraction and Claude 3P default path
...
- df66940: Fix `install` silently skipping the target agent when the canonical skill directory already exists but the lock file is missing or holds a different version.
...
- ea6ac26: fix(core): make "already installed" check per-agent instead of global
...
- 05f7a79: Update reskill-usage skill documentation
  ...
  Add `list -a` option, Claude Cowork 3P agent support, and global install scope to the reskill-usage SKILL.md.

  ***

  更新 reskill-usage skill 文档

  添加 `list -a` 选项、Claude Cowork 3P agent 支持以及全局安装作用域到 reskill-usage SKILL.md。
...
- 90a2986: Add experimental Claude Cowork 3P support, including app-managed skills root discovery, manifest updates, copy-based installation, uninstall/list support, safety checks for skill paths, and README documentation marked as Beta.
...
- 853e589: Tolerate empty `x-integrity` header from registry for local-mode publishes.
...
- ed0c4b0: Fix `install` failing on Git repositories whose default branch is not `main`.
